<template>
  <v-container fluid>
    ...
    <div v-if="!showFilterList.show">
      <v-card class="mt-3 mb-5" color="grey lighten-2" v-for="ad in garbageAds" :key="ad.id">
    ...
    <div v-if="garbageAds.length == 0  || showFilterList.empty" class="mt-8 text-center">
    ...
    <v-dialog v-model="showInfo">
      <v-card>
        ...
        <v-card-text>
          <p class="mb-2">Anuncie aqui resíduos para descarte.</p>
          <p class="mb-2 text-justify">Além de incentivar a coleta seletiva, o munícipe não precisa esperar pelos dias e horários estabelecidos pelo caminhão de lixo.</p>
          <center>
          <v-row>
            <v-col cols="3">
...
<script>
import { auth, storageRef, garbageAdsCollection } from '@/firebase'
...
export default {
  ...
  computed: {
    ...mapState(['districts', 'garbageAds', 'userProfile'])
  },
  methods: {

    async announce() {
      await garbageAdsCollection.add({
        createdAt: new Date(),
        userId: auth.currentUser.uid,
    ...
    async delAd(ad) {
      await garbageAdsCollection.doc(ad.id).delete()
        .then(function() {
        })
        .catch(function(e) {
          console.log("ERRO" + e)
    ...
    uploadPhotos(e) {
      ...
      e.forEach((doc, key) => {
        this.uploadTask = storageRef.child('garbagePictures/' + '/' + auth.currentUser.uid + '/' + paste + '/' + Math.floor(Math.random() * 65536)).put(doc)
          .then(response => {
    ...
    async filter(ad) {
      var query = garbageAdsCollection

      if(ad === 'others') {
        query = garbageAdsCollection
